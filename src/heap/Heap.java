...
public class Heap<E> extends LinkedBinaryTree<E> {
	...
	private HeapPosition<E> validate(Position<E> p) throws IllegalArgumentException { 
		if (!(p instanceof HeapPosition<?>)) throw new IllegalArgumentException("Invalid position - not of type HeapPosition."); 
		HeapPosition<E> hp = (HeapPosition<E>) p; 
		if (hp.getIndex() < 0 || hp.getIndex() >= list.size()) throw new IllegalArgumentException("Position does not belong to this heap."); 
		return hp; 	
	}
