...
public abstract class AbstractBinaryTree<E> extends AbstractTree<E> implements
	...
	// The following part has to do with Exercise 3. 
		...
		protected void recDisplay(Position<E> root,	int[] control, int level) {
			
			String name ;
			if(level == 0) name = "ROOT";
			else {
				if(this.left(parent(root)).equals(root)) {name = "L";} 
				else name = "R"; 
			}
			//Old Code
			...
			System.out.println();
			printPrefix(level, control);
			System.out.println("__" + name + "("+root.getElement()+")");
			control[level]--;
