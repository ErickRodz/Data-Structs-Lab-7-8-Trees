...
public class LinkedTree<E> extends AbstractTree<E> implements Cloneable {
	...
	private Node<E> validate(Position<E> p) throws IllegalArgumentException { 
		...
		Node<E> np = (Node<E>) p; 
		if (np.getParent() == np) { throw new IllegalArgumentException("Target position is not part of a tree."); }
		
		// the following validates that p is a position in this tree
		if (np.getOwnerTree() != this) { throw new IllegalArgumentException("Target position is not part of the tree."); }	
		return np; 
	}
	...
	public E remove(Position<E> p) throws IllegalArgumentException { 
		Node<E> ntr = validate(p); 
		E etr = ntr.getElement(); 
		Node<E> parent = ntr.getParent(); 
		if (parent == null)    
			if (numChildren(ntr) > 1) { throw new IllegalArgumentException("Cannot remove."); }
			else if (numChildren(ntr) == 0) { 
				root = null; 
				} else { 
				root = ntr.getChildren().get(0); 
				root.setParent(null);
			}
		else { 
			for (Node<E> childNTD : ntr.children) { 
				parent.getChildren().add(childNTD);
				childNTD.setParent(parent); 
			}	
		}
		ntr.discard();
		parent.children.remove(ntr);
